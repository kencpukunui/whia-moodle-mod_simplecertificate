...
class simplecertificate {
     ...
     * 
     * Changes by Pukunui - BUW-448702
     * 26062018 modify the simplecertificate insert date to module completion date
     * To trigger it, set the restriction of your simplecertificate in the course
     * If the condition has been satisfied, it will choose the biggest "timemodified" field 
     * from mdl_course_modules_completion database of this course and replace the simplecertificate date
     * 29062018 Get existing user's profile field from two tables mdl_user_info_data and mdl_user_info_field
     * Pass the profile data to print on PDF, remember go to this path /home/ken/public_html/mod/simplecertificate/lang/en/simplecertificate.php
     * Change the parameter {PROFILE_xxxx} to whatever you want, I have changed it to be {PROFILE_AGY}
     * Moodle will auto detect your changes and generate a new lowercase name called profile_AGY in the $a array
     */
    protected function get_certificate_text($issuecert, $certtext = null) {
        ...
        // Start - Changes by Pukunui - BUW-448702.
        //$a->date = $this->get_date($issuecert, $user->id);
        //Get the final module completion of this course
        $courseid = $this->get_instance()->course;
        $sql = "SELECT FROM_UNIXTIME(A.date, '%d-%m-%Y')
                FROM (SELECT  MAX(cmc.timemodified) AS 'date'
                      FROM mdl_course_modules cm
                      INNER JOIN mdl_course_modules_completion cmc
                      ON cm.id = cmc.coursemoduleid
                      WHERE cm.course = '$courseid'
                      AND cmc.userid = '$user->id') AS A";
        $completiondate = $DB->get_field_sql($sql);
        $a->date = $completiondate;

        //Get the existing profile data from user 
        $accsql = "SELECT uid.data
                   FROM mdl_user_info_data uid
                   INNER JOIN mdl_user_info_field uif
                   ON uid.fieldid = uif.id
                   WHERE uif.shortname = 'AGY'
                   AND uid.userid = $user->id";
        $getaccfield = $DB->get_field_sql($accsql);
        if ($getaccfield) {
            $a->profile_AGY = strip_tags($getaccfield);
        }
        else {
            $a->profile_AGY = strip_tags("");
        }
        // End - Changes by Pukunui - BUW-448702.
